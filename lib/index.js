...
class Qingya {
    ...
    initgit() {
        if (!fs.existsSync(this.config.public_dir)) {
            fs.mkdir(this.config.public_dir);
        }
    ...
    compilePost(filename) {
        ...
        return new Promise((resolve, reject) => {
            ...
            fs.readFile(file, 'utf8', (err, data) => {
                ...
                // 根据模板生成html
                var html = template(this.path('template/post.art'), {
                    type: 'post',
    ...
    compileIndex(posts, pages) {
        ...
        // 根据模板生成html
        var html = template(this.path('template/index.art'), {
            posts,
            pages,
            config: this.config
        });
